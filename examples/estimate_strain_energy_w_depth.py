    ...
    for i, depth in enumerate(odepths):
        ...
        outs.append(
            pysra.output.StressTSOutput(
                pysra.output.OutputLocation("within", depth=depth), normalized=False
            )
        )
...
def calc_case_and_cake_from_lin_analysis(sp, m):
    ...
    for i, depth in enumerate(odepths):
        ...
        down_acc_signal = eqsig.AccSignal(
            (oseries["upACCX_d%i" % i] - oseries["ACCX_d%i" % i]) * 9.8, m.time_step
        )
...
def create(show=0):
    ...
    m = pysra.motion.TimeSeriesMotion(
        "test motion", description=None, time_step=dt, accels=acc / 9.8
    )

    # Calculate the input CAKE to normalise the plots
    in_signal = eqsig.AccSignal(
        (m.accels / 2) * 9.8, m.time_step
    )  # Should be input as m/s2 (note divide by 2)
    ...
    sps[0].plot(
        ...
        odepths,
        ls="-",
        label="Linear analysis $\\xi={0:.0f}$%".format(damp * 100),
        c=cbox(0),
        lw=1,
    )
    sps[1].plot(
    ...
    pred_case = lq.trigger.nses.est_case_1d_millen_et_al_2019(
        sp, in_signal, odepths, xi=damp
    )[:, -1]
    pred_cake = lq.trigger.nses.est_case_1d_millen_et_al_2019(
    ...
    pred_case = lq.trigger.nses.est_case_1d_millen_et_al_2019(
        sp, surf_sig, odepths, xi=damp, in_loc=0
    )[:, -1]

    pred_cake = lq.trigger.nses.est_case_1d_millen_et_al_2019(
        sp, surf_sig, odepths, xi=damp, in_loc=0, nodal=False
    )[:, -1]
    sps[0].plot(
        pred_case / in_cake, odepths, "x", ms=3, label="NSES Estimate (surf)", c=cbox(3)
    )
    sps[1].plot(
        pred_cake / in_cake, odepths, "x", ms=3, label="ASES Estimate (surf)", c=cbox(3)
    )
