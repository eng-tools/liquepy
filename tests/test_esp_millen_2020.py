...
def test_fit_single_layer():
    ...
    d_liqs, d_nonliqs, csr_n15s, normed_diff = lq.esp.millen_2020.fit_n_layer_profile(
        depths, crr, n=3, crr_n15_opts=crr_opts
    )
    ...
    d_liqs, d_nonliqs, csr_n15s, normed_diff = lq.esp.millen_2020.fit_n_layer_profile(
        depths, crr, n=5, crr_n15_opts=crr_opts
    )
...
def disable_test_fit_3layer_from_cpt():
    ...
    bi2014 = lq.trigger.run_bi2014(
        cpt, pga=0.25, m_w=7.5, gwl=cpt.gwl, unit_wt_method="void_ratio"
    )

    n_layers = 3
    d_liqs, d_nonliqs, csr_n15s, normed_diff = lq.esp.millen_2020.fit_n_layer_profile(
        bi2014.depth, bi2014.crr_m7p5, n=n_layers
    )
...
def disabled_test_fit_5layer_from_cpt():
    ...
    if show:
        ...
        sps[0].plot(
            [d_liqs[0], d_nonliqs[0]], [csr_n15s[0], csr_n15s[0]], c="k", ls="--"
        )
        if n_layers == 5:
            sps[0].plot(
                [d_liqs[1], d_nonliqs[1]], [csr_n15s[1], csr_n15s[1]], c="k", ls="--"
            )
...
def run():
    ...
    for nstep in nsteps:
        ...
        d_liqs, d_nonliqs, csr_n15s, normed_diff = (
            lq.esp.millen_2020.fit_n_layer_profile(
                bi2014.depth, bi2014.crr_m7p5, n=n_layers, crr_n15_opts=crr_opts
            )
        )
