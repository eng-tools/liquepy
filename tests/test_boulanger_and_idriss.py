...
def test_can_calculate_fos():
    ...
    bi2014 = liquepy.trigger.run_bi2014(
        cpt, pga=0.25, m_w=7.5, gwl=cpt.gwl, unit_wt_method="robertson2009"
    )
...
def test_compare_fos_to_previous_version():
    ...
    np.savetxt(
        TEST_DATA_DIR + "standard_1_fos_lq%s.csv" % new_version, factor_safety_values
    )
...
def test_crr_7p5_from_cpt():
    ...
    assert (
        bim14.calc_crr_m7p5_from_qc1ncs_capped(q_c1n_cs, gwl=6.0, depth=depth, i_c=1.8)
        == 4.0
    )
    assert (
        bim14.calc_crr_m7p5_from_qc1ncs_capped(q_c1n_cs, gwl=1.0, depth=depth, i_c=2.7)
        == 4.0
    )
    assert (
        bim14.calc_crr_m7p5_from_qc1ncs_capped(
            q_c1n_cs, gwl=1.0, depth=depth, i_c=2.5, i_c_limit=2.4
        )
        == 4.0
    )
...
def test_calculate_qc_1ncs_from_crr_7p5():
    ...
    crr_values = bim14.calc_crr_m7p5_from_qc1ncs_capped(
        q_c1n_cs_values, depth=10, gwl=0, i_c=1.8
    )
...
def test_handles_predrill():
    ...
    unit_wt = liquepy.trigger.boulanger_and_idriss_2014.calc_unit_dry_weight(
        cpt.f_s, cpt.q_t, p_a=101, unit_water_wt=9.8
    )
...
def test_calc_crr_m7p5_from_n1_60cs():
    crr = liquepy.trigger.boulanger_and_idriss_2014.calc_crr_m7p5_from_n1_60cs(
        3, c_0=2.8
    )
