...
def calc_volumetric_strain_zhang_2002(factor_of_safety, q_c1n_cs):
    ...
    """
    if isinstance(factor_of_safety, numbers.Real) and isinstance(
        q_c1n_cs, numbers.Real
    ):
        return _calc_single_volumetric_strain(factor_of_safety, q_c1n_cs)
    elif not isinstance(q_c1n_cs, numbers.Real) and not isinstance(
        factor_of_safety, numbers.Real
    ):
...
def _calc_single_volumetric_strain(factor_of_safety, q_c1n_cs):
    ...
    fs_values = [0.5, 0.6, 0.7, 0.8, 0.9, 1, 1.1, 1.2, 1.3, 2, 300.0]
    for i in range(len(fs_values)):
        if factor_of_safety < fs_values[i]:
            ...
            ev_actual = np.interp(
                factor_of_safety, [low_fs, high_fs], [ev_low, ev_high]
            )
            return ev_actual
