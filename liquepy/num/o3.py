...
class PM4Sand(PM4SandBase):
    ...
    def __repr__(self):
        return "PM4SandO3 Soil model, id=%i, phi=%.1f, Dr=%.2f" % (
            self.id,
            self.phi,
...
class PM4Silt(PM4SiltBase):
    ...
    def __repr__(self):
        return "PM4SiltO3 Soil model, id=%i, phi=%.1f, Dr=%.2f" % (
            self.id,
            self.phi,
            self.relative_density,
        )
...
class ManzariDafaliasModel(models.ManzariDafaliasModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        models.ManzariDafaliasModel.__init__(
            self,
            wmd=wmd,
            liq_mass_density=liq_mass_density,
            liq_sg=liq_sg,
        ...
        self._extra_class_inputs = []
        self.app2mod = {"den": "unit_moist_mass", "nu": "poissons_ratio"}
...
class StressDensityModel(StressDensityModelBase):
    ...
    def __repr__(self):
        return "StressDensityModelO3 Soil model, id=%i, phi=%.1f, Dr=%.2f" % (
            self.id,
            self.phi,
            self.relative_density,
        )
