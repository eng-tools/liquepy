...
def build_cmaps():
    ...
    FS_CMAP = LinearSegmentedColormap.from_list(
        "fs", [FS_COLORS[cname] for cname in FS_COLORS], N=5
    )
    FS_CMAP_W_NONLIQ = LinearSegmentedColormap.from_list(
        "fs_w_nonliq", [FS_COLORS_W_NONLIQ[cname] for cname in FS_COLORS_W_NONLIQ], N=5
    )
    _incs = np.round(np.diff(IC_LIMITS) * 10).astype(int)
    _clist = list(
        itertools.chain(
            *[[IC_COLORS[cname]] * _incs[x] for x, cname in enumerate(IC_COLORS)]
        )
    )
    ...
    _clist = list(
        itertools.chain(
            *[
                [IC_COLORS_w_CONTRAST[cname]] * _incs[x]
                for x, cname in enumerate(IC_COLORS_w_CONTRAST)
            ]
        )
    )
    IC_CMAP_w_CONTRAST = LinearSegmentedColormap.from_list(
        "mine", _clist, N=len(_clist)
    )
...
def make_factor_of_safety_plot(subplot, w_ic_lim=False):
    # add the Fs = 1 line
    subplot.axvspan(0.0, 0.75, alpha=0.5, color=FS_LOW_to_0p75)
...
def make_bi2014_outputs_plot(sps, bi2014, crr_cap=0.6):
    ...
    sps[0].plot(
        bi2014.sigma_veff, bi2014.depth, lw=1, c="gray", label="$sigma_{v,eff}$"
    )
    ...
    sps[2].plot(
        np.clip(bi2014.crr_m7p5, None, crr_cap),
        bi2014.depth,
        "o",
        lw=1,
        c="k",
    ...
    sps[2].set_xlim([0, 1.0])

    sps[3].plot(
        bi2014.factor_of_safety, bi2014.depth, "o", lw=1, c="k", alpha=0.5, ms=3
    )
